...
GISTLIST_DIR_NAME = "gistlist"
FETCH_YAML_NAME = "fetch.yaml"
...
def reconcile_fetch_entries(
    ...
    `fetch_assoc` に存在しないカウントは、現在時刻と 0 件を既定値として補う。
...
class CommandFix(Command):
    """`gistlist/` 配下の空ディレクトリと `fetch.yaml` の整合性を修復する。"""
    ...
    def _reconcile_fetch_yaml(self, fetch_path: Path, existing_counts: list[int]) -> None:
        """既存 gistlist カウント一覧に合わせて `fetch.yaml` を保存し直す。"""
        ...
        Loggerx.debug(
            f"CommandFix._reconcile_fetch_yaml | fetch.yaml updated, counts={existing_counts}",
            __name__,
        )
    ...
    def _get_workspace_path(self) -> Path:
        """現在ユーザに対応するワークスペースパスを OS ごとに解決する。"""
